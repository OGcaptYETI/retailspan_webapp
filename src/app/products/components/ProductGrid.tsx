...
export function ProductGrid({
  ...
  const filteredProducts = React.useMemo(() => {
    ...
    return products.filter((product) =>
      ["name", "brand", "category"]
        .map((key) => (product[key as keyof typeof product]?.toString().toLowerCase().includes(lowerSearchTerm)))
        .some(Boolean)
    );
  }, [products, searchTerm]);
  ...
  return (
    <div className={cn("space-y-6", className)}>
      <div className="flex items-center justify-between">
        ...
        />
        <Text className="text-muted-foreground">{filteredProducts.length} products</Text>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <button
            ...
            onClick={() => alert("Create a new product")}
          >
            Create New Product
          </button>
        </div>
